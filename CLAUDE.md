...
- Onboarding wizard state is managed in the parent page component and passed down via props
...
- Validate API inputs with Zod before database operations
...
- Zod schemas validate all POST endpoints; return 400 with `fieldErrors` on failure

## Known Issues
- No RLS on any Supabase tables (by design — no auth in this app)
- `SUPABASE_SERVICE_ROLE_KEY` needs to be set in `.env.local` for production; currently falls back to anon key
- The trigger function `update_updated_at_column` has a mutable search_path (Supabase lint warning, low priority)
- StepDemo directly mutates `data.orgId` instead of using `updateData()` — works but is a React anti-pattern
